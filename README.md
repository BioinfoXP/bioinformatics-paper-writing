...
- Outputs and analysis flow

This is useful when the manuscript draft and the real code have drifted apart.
...
- Start with `Figure X.` and one short message sentence
- Insert one blank line after the opening sentence
- Write one panel per line
- Use `(A)`, `(B)`, `(C)` formatting
- End each panel sentence with a period
- Avoid semicolon-chained panel descriptions
...
### Write Results from a figure

```text
Use $bioinformatics-paper-writing to write Results from this figure.
```

### Write Methods from code

```text
Use $bioinformatics-paper-writing to write the Methods section from this analysis code.
```

### Learn from a paper

```text
Use $bioinformatics-paper-writing to learn reusable writing patterns from this paper.
```
...
By default, the skill writes like manuscript copy rather than a working memo.
...
This skill does not replace domain judgment. It is strongest when:

- Figure labels are readable
- Comparison groups are visible
- Code is complete enough to reconstruct methods
- The user supplies missing context when the figure alone is ambiguous
...
如果你愿意，我下一步可以直接把这份 README 写成仓库根目录的 [README.md](/C:/Users/Wandering/.codex/skills/bioinformatics-paper-writing/README.md) 版本。
